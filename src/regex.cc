...
Regexv2::Regexv2(const std::string& pattern_, int debuglevel, bool use_jit): RegexBase::RegexBase(pattern_, debuglevel) {
    ...
    if (m_pc != NULL) {
        if (use_jit) {
            ...
            if (jit_enabled == 1) {
                pcre2_jit_compile(m_pc, PCRE2_JIT_COMPLETE);
            }
        }
    }

    if (m_pc != NULL && debuglevel > 0) {
        pcre2_code *re = m_pc;
        printf("\nDETAILED INFORMATION:\n=====================\n");
        _DISPLAY_PCRE2_PROP(PCRE2_INFO_BACKREFMAX);
        _DISPLAY_PCRE2_PROP(PCRE2_INFO_CAPTURECOUNT);
        _DISPLAY_PCRE2_PROP_U(PCRE2_INFO_DEPTHLIMIT);
        _DISPLAY_PCRE2_PROP_UL(PCRE2_INFO_JITSIZE);
        _DISPLAY_PCRE2_PROP(PCRE2_INFO_MINLENGTH);
        _DISPLAY_PCRE2_PROP_U(PCRE2_INFO_MATCHLIMIT);
        _DISPLAY_PCRE2_PROP(PCRE2_INFO_SIZE);
    }
